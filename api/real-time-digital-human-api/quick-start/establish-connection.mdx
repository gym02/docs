...
<Note>
    Before `session.created` arrives, you will always receive a `session.session_id` event. Capture the `session_id` value as soon as it arrives because you must reuse it later when opening the WebRTC signaling socket. Treat this value as an opaque ID generated by the proxy session.
  </Note>
...
async function handleReceivedMessage(data) {
  switch (data.type) {
    ...
    case 'session.session_id':
      console.log('Received proxy session ID:', data.session_id);
      // Store this ID for later WebRTC signaling (see Step 5)
      // You can also reuse it as `targetSessionId = target-${session_id}` when creating the WebRTC connection
      break;
...
```
  ...
  <Note>
    ...
    1. `session.session_id` → The server sends the proxy session ID that you must use to authorize your WebRTC signaling connection
    2. `session.created` → Send configuration
    3. `session.updated` → Start sending audio
    4. `input_audio_buffer.speech_started` → User starts speaking
    5. `input_audio_buffer.speech_stopped` → User stops speaking
    6. `conversation.item.input_audio_transcription.completed` → User speech transcribed
    7. `response.audio_transcript.delta` → AI response text (streaming, multiple events)
    8. `response.audio_transcript.done` → AI response text complete
    9. `response.audio.done` → AI response audio complete
  </Note>
</Step>
...
<Step title="Step 5: Establish WebRTC Connection (for Video)">
  ...
  <Note>
    As soon as you receive the `session.session_id` event on the WebSocket channel, you may open the WebRTC signaling socket. Use that session ID as the `userId` query parameter and set `targetSessionId = "target-{session_id}"` in the `{ type: 'create' }` message so the video connection binds to that proxy session.
  </Note>
</Step>
</Steps>
