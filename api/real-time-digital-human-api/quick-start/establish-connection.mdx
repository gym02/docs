...
<Step title="Configure Session">
  After the connection is established, configure the session with your preferences. For detailed configuration options and parameter descriptions, see [session.created event documentation](/api/real-time-digital-human-api/websocket-events/session-events/session-created).
