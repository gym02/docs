...
* **Intelligent Parameter Configuration**: Simplified connection parameters with default value mechanism. Only `license` and `name` are required; `model` is optional.
...
<Warning>
This is a **breaking change**. The new API (v2) is not backward compatible with the old version (v1). Please refer to the [Migration Guide](/api/real-time-digital-human-api/migration-guide) for detailed migration instructions.
</Warning>
</Tab>
